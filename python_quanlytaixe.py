...
def create_quanlytaixe_window(parent=None):
    root2 = tk.Toplevel(parent) if parent is not None else tk.Toplevel()
    root2.title("Quản lý Tài Xế")
    center_window(root2, 1000, 600)
    ...
    tk.Button(frame_btn, text="Thoát", width=8, command=root2.withdraw).grid(row=0, column=5, padx=5)
